...
def load_image(path):
    ...
    if os.path.exists(path):
        try:
            ...
            raise LoadImageError('IOError: %s' % e)
    else:
        raise LoadImageError('"%s" not found' % path)
    ...
    if image.mode in ['L', 'RGB']:
        ...
        raise LoadImageError('Image mode "%s" not supported' % image.mode)
...
def embed_image_html(image):
    ...
    data = base64.b64encode(string_buf.getvalue()).decode('ascii').replace('\n', '')
